...
/**
 * Finder implements basic methods for {@link IFinder} leaving getAll() and _toString() methods to concrete classes.
 *
...
public abstract class Finder implements IFinder {
     ...
     *
     * It is necessary to provide this renderer since by default a ComboBox
     * sets label as Objects.toString(data) which relies on the actual implementation of Object.toString()
     * By doing this, it is possible to decouple how an object is shown from its Object.toString() method.
     *
     * In general it would not be necessary to overwrite this Renderer.
     ...
     * This method searches among all elements from model whether an input text (value) matches or not.
     *
     * In case that element matches,
     * it will be added to list, and a new SimpleListModelExt if returned out of that list.
     *
     * @author Diego Pino Garcia<dpino@igalia.com>
     */
