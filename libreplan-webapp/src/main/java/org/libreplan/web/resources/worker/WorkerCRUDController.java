...
public class WorkerCRUDController extends GenericForwardComposer implements IWorkerCRUDControllerEntryPoints {
    ...
    private enum UserBindingOption {
        NOT_BOUND(_t("Not bound")),
        EXISTING_USER(_t("Existing user")),
        CREATE_NEW_USER(_t("Create new user"));
     ...
     */
    public boolean isWorkerCodeAutogenerated() {
        return getWorker().isCodeAutogenerated();
    }

    public void setWorkerCodeAutogenerated(boolean codeAutogenerated) {
        getWorker().setCodeAutogenerated(codeAutogenerated);
    }
    ...
    public org.zkoss.zul.ListModel getRealWorkers() {
        return new ListModelList<>(workerModel.getRealWorkers());
    }

    public org.zkoss.zul.ListModel getVirtualWorkers() {
        return new ListModelList<>(workerModel.getVirtualWorkers());
    }
    ...
    public boolean save() {
        ...
        try {
            ...
            workerModel.save();
            messages.showMessage(Level.INFO, _t("Worker saved"));
    ...
    private void setUserBindingInfo() {
        ...
        if (UserBindingOption.EXISTING_USER.ordinal() == option) {
            if (getWorker().getUser() == null) {
                throw new WrongValueException(userBandbox, _t("please select a user to bound"));
            }
    ...
    private User createNewUserForBinding() {
        ...
        if (StringUtils.isBlank(loginName)) {
            throw new WrongValueException(loginNameTextbox, _t("cannot be empty"));
        }

        String password = passwordTextbox.getValue();
        if (StringUtils.isBlank(loginName)) {
            throw new WrongValueException(passwordTextbox, _t("cannot be empty"));
        }

        String passwordConfirmation = passwordConfirmationTextbox.getValue();
        if (!password.equals(passwordConfirmation)) {
            throw new WrongValueException(passwordConfirmationTextbox, _t("passwords do not match"));
        }
    ...
    public void goToEditForm(Worker worker) {
        ...
        showEditWindow(_t("Edit Worker: {0}", worker.getHumanId()));
    ...
    public void goToEditVirtualWorkerForm(Worker worker) {
        ...
        editAssignedCriterions();
        showEditWindow(_t("Edit Virtual Workers Group: {0}", worker.getHumanId()));
    }
    ...
    public void goToEditForm() {
        ...
        showEditWindow(_t("Edit Worker: {0}", getWorker().getHumanId()));
    }
    ...
    public void goToCreateForm() {
        ...
        resourcesCostCategoryAssignmentController.setResource(workerModel.getWorker());
        updateUserBindingComponents();
        showEditWindow(_t("Create Worker"));
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        // The register() call below synchronously invokes goToEditForm()/goToCreateForm() (via
        // EntryPointsHandler, the normal way this page is reached with a "?edit=..." URL),
        // wrapped in Util.executeIgnoringCreationOfBindings(...) - which only guards
        // Util.createBindingsFor, not Util.reloadBindings. BaseCRUDController's own
        // showEditWindow() (called from within that same synchronous flow) already calls
        // Util.reloadBindings(editWindow) itself, so as long as the binder already exists by the
        // time that runs, it works correctly. Create the binder here, before register(), and
        // reload listWindow immediately (its bindings don't reference the not-yet-populated edit
        // entity) - editWindow's own reload is handled later by showEditWindow().
    ...
    private void initUserBindingComponents() {
        ...
        userBandbox.setListboxEventListener(Events.ON_SELECT, event -> {
            Listitem selectedItem = (Listitem) ((SelectEvent) event).getSelectedItems().iterator().next();
            setBoundUser(selectedItem.getValue());
        });
    ...
    private void initUserBindingOptions() {
        ...
        for (UserBindingOption option : values) {
            ...
            if ( option.equals(UserBindingOption.CREATE_NEW_USER) &&
                ...
                radio.setTooltiptext(_t("You do not have permissions to create new users"));
            }
    ...
    public org.zkoss.zul.ListModel getBaseCalendars() {
        return new ListModelList<>(workerModel.getBaseCalendars());
    }
    ...
    /**
     * calendars/_edition.zul is a shared macro, embedded here inline inside the Worker edit
     * form's "Calendar" tab (as opposed to being driven by its own standalone page). Every
     * binding inside that embedded subtree is still registered against the SAME single
     * page-wide binder as the rest of this form (AnnotateBinder captures its controller once,
     * at binding-registration time - it does not re-resolve "controller" freshly per component),
     * so "controller.baseCalendarCodeAutogenerated" has to be reachable from WorkerCRUDController
     * itself, not from baseCalendarEditionController. Delegates to the same logic
     * BaseCalendarEditionController already exposes for its OTHER embedding context
     * (MachineCRUDController's own Calendar tab).
     */
    public boolean isBaseCalendarCodeAutogenerated() {
        return baseCalendarEditionController != null && baseCalendarEditionController.isBaseCalendarCodeAutogenerated();
    }

    public void setBaseCalendarCodeAutogenerated(boolean codeAutogenerated) {
        if ( baseCalendarEditionController != null ) {
            baseCalendarEditionController.setBaseCalendarCodeAutogenerated(codeAutogenerated);
        }
    }
    ...
    public void goToCreateVirtualWorkerForm() {
        ...
        createAssignedCriterions();
        resourcesCostCategoryAssignmentController.setResource(workerModel.getWorker());
        showEditWindow(_t("Create Virtual Workers Group"));
        resourceCalendarModel.cancel();
    }
    ...
    public org.zkoss.zul.ListModel getLimitingResourceOptionList() {
        return new SimpleListModel<>(new ArrayList<>(LimitingResourceEnum.getLimitingResourceOptionList()));
    }
        ...
        try {
            if (!workerModel.canRemove(worker)) {
                messages.showMessage(
                        Level.WARNING,
                        _t("This worker cannot be deleted because it has assignments to projects or imputed hours"));
                return;
            }

            int status = Messagebox.show(
                    _t("Confirm deleting this worker. Are you sure?"),
            ...
            if (user != null && !user.isSuperuser()) {
                ...
                removeBoundUser = Messagebox.show(
                        _t("Do you want to remove bound user \"{0}\" too?", user.getLoginName()),
                        _t("Delete bound user"),
                        Messagebox.YES | Messagebox.NO, Messagebox.QUESTION) == Messagebox.YES;
            }

            workerModel.confirmRemove(worker, removeBoundUser);
            messages.showMessage(Level.INFO,
                    removeBoundUser ? _t("Worker and bound user deleted") : _t("Worker deleted"));
            goToList();
        } catch (InstanceNotFoundException e) {
            messages.showMessage(Level.INFO, _t("This worker was already removed by other user"));
        }
    }
    ...
    public RowRenderer getWorkersRenderer() {
        return (row, data, i) -> {
            ...
            row.appendChild(new Label(Boolean.TRUE.equals(worker.isLimitingResource()) ? _t("yes") : _t("no")));
     ...
     * iteration. Building rows programmatically via RowRenderer, like getWorkersRenderer() above,
     * sidesteps that "each" bug entirely.
     */
    public RowRenderer getVirtualWorkersRenderer() {
        return (row, data, i) -> {
            final Worker virtualWorker = (Worker) data;
            row.setValue(virtualWorker);

            row.addEventListener(Events.ON_CLICK, event -> goToEditVirtualWorkerForm(virtualWorker));

            row.appendChild(new Label(virtualWorker.getFirstName()));
            String capacity = virtualWorker.getCalendar() != null
                    ? String.valueOf(virtualWorker.getCalendar().getCapacity())
                    : "";
            row.appendChild(new Label(capacity));
            Label observations = new Label(((VirtualWorker) virtualWorker).getObservations());
            observations.setMaxlength(40);
            row.appendChild(observations);

            Hbox hbox = new Hbox();
            hbox.appendChild(Util.createEditButton(event -> goToEditVirtualWorkerForm(virtualWorker)));
            hbox.appendChild(Util.createRemoveButton(event -> confirmRemove(virtualWorker)));
            row.appendChild(hbox);
        };
    }
    ...
    public void updateWindowTitle() {
        if (editWindow != null && state != CRUDControllerState.LIST) {
            ...
            String entityType = _t("Worker");
            if (worker.isVirtual()) {
                entityType = _t("Virtual Workers Group");
            }
    ...
    private int showConfirmUserEditionDialog() {
        return Messagebox.show(
                ...
                _t("Confirm editing user"), Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
    }
    ...
    public String getUserEditionButtonTooltip() {
        return isNoRoleUserAccounts()
                ? _t("You do not have permissions to go to edit user window")
                : "";
    }
