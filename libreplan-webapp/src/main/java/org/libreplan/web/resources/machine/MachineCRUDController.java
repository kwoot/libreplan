...
public class MachineCRUDController extends BaseCRUDController<Machine> {
    ...
    public org.zkoss.zul.ListModel getMachines() {
        return new ListModelList<>(machineModel.getMachines());
    }
     ...
     */
    public boolean isMachineCodeAutogenerated() {
        return getMachine().isCodeAutogenerated();
    }

    public void setMachineCodeAutogenerated(boolean codeAutogenerated) {
        getMachine().setCodeAutogenerated(codeAutogenerated);
    }

    /**
     * calendars/_edition.zul is a shared macro, embedded here inline inside the Machine edit
     * form's "Calendar" tab. Every binding inside that embedded subtree is registered against
     * the SAME single page-wide binder as the rest of this form (AnnotateBinder captures its
     * controller once, at binding-registration time - it does not re-resolve "controller"
     * freshly per component), so "controller.baseCalendarCodeAutogenerated" has to be reachable
     * from MachineCRUDController itself, not from baseCalendarEditionController. Same fix as
     * WorkerCRUDController's own Calendar tab - see zul-sweep-log.md for the full investigation.
     */
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        // controller.machine, still null at this point (only set up by initCreate()/initEdit(),
        // called later); showEditWindow() reloads editWindow's own bindings once that happens.
    ...
    public org.zkoss.zul.ListModel getBaseCalendars() {
        return new ListModelList<>(machineModel.getBaseCalendars());
    }
