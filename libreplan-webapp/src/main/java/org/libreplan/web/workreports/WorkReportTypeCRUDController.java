...
public class WorkReportTypeCRUDController
    ...
    public List<WorkReportType> getWorkReportTypes() {
        ...
        return new org.zkoss.zul.ListModelList<>(workReportTypeModel.getWorkReportTypesExceptPersonalAndJiraTimesheets());
    }
     ...
     * entirely, the same way the descriptionField/workReportLabelTypeAssignment/
     * orderedFieldsAndLabels renderers below already do for the edit form's grids.
     */
    public org.zkoss.zul.RowRenderer getWorkReportTypeRowRenderer() {
        return (row, data, index) -> {
            final WorkReportType workReportType = (WorkReportType) data;
            row.setValue(workReportType);

            row.appendChild(new org.zkoss.zul.Label(workReportType.getName()));

            org.zkoss.zul.Hbox hbox = new org.zkoss.zul.Hbox();
            hbox.appendChild(Util.createEditButton(event -> goToEditForm(workReportType)));
            hbox.appendChild(Util.createRemoveButton(event -> confirmDelete(workReportType)));

            org.zkoss.zul.Button newTimesheetButton = new org.zkoss.zul.Button(_t("New timesheet"));
            newTimesheetButton.setSclass("add-button");
            newTimesheetButton.addEventListener(
                    org.zkoss.zk.ui.event.Events.ON_CLICK, event -> goToEditNewWorkReportForm(workReportType));
            hbox.appendChild(newTimesheetButton);
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        // controller.workReportType (isReadOnly()/isEditable() call thereAreWorkReportsFor(null)
        // on it), which is still null at this point (only set up by initCreate()/initEdit(),
        // called later); showEditWindow() reloads editWindow's own bindings once that has happened.
     ...
     */
    public boolean isWorkReportTypeCodeAutogenerated() {
        return getWorkReportType().isCodeAutogenerated();
    }

    public void setWorkReportTypeCodeAutogenerated(boolean codeAutogenerated) {
        getWorkReportType().setCodeAutogenerated(codeAutogenerated);
    }
    ...
    public List<DescriptionField> getDescriptionFields() {
        ...
        // no automatic List->ListModel coercion, so returning a plain List here throws a
        // ClassCastException when the binder tries to load it.
        return new org.zkoss.zul.ListModelList<>(workReportTypeModel.getDescriptionFields());
    }
    ...
    public List<WorkReportLabelTypeAssignment> getWorkReportLabelTypeAssignments() {
        // The grid's "model" property is declared as org.zkoss.zul.ListModel - AnnotateBinder has
        // no automatic coercion from Set (or List) to ListModel, so wrap it explicitly.
        return new org.zkoss.zul.ListModelList<>(workReportTypeModel.getWorkReportLabelTypeAssignments());
    }
    ...
    // Separate wrapper for the listbox's "model" binding - it's declared as org.zkoss.zul.ListModel
    // and AnnotateBinder has no automatic array->ListModel coercion. getHoursManagementEnums()
    // itself is kept array-returning since createListPosition() (below) already wraps it in its
    // own SimpleListModel for a different, programmatically-built listbox.
    public org.zkoss.zul.ListModel getHoursManagementEnumsModel() {
        return new org.zkoss.zul.ListModelArray<>(getHoursManagementEnums());
    }
    ...
    // Separate wrapper for the listboxes' "model" binding - see getHoursManagementEnumsModel().
    public org.zkoss.zul.ListModel getPositionInWorkReportEnumsModel() {
        return new org.zkoss.zul.ListModelArray<>(getPositionInWorkReportEnums());
    }
    ...
    public void validateIfExistTheSameLabelType(
        ...
        if ( (getWorkReportType() != null) &&
            ...
            throw new WrongValueException(comboLabelTypes, _t("Label type already assigned"));
        }
    }
    ...
    public Constraint validateIfExistTheSameFieldName(final DescriptionField descriptionField) {
        return (comp, value) -> {
            ...
            if ((getWorkReportType() != null) && (getWorkReportType().existSameFieldName(descriptionField))) {
                ...
                throw new WrongValueException(comp, _t("A description field with the same name already exists."));
            }
        };
    }
    ...
    private void showInvalidDescriptionFieldName(DescriptionField field) {
        ...
        throw new WrongValueException(fieldName, _t("The field name must be unique and not empty"));
    }
    ...
    private void showInvalidDescriptionFieldLength(DescriptionField field) {
        ...
        throw new WrongValueException(fieldName, _t("The length must be greater than 0 and not empty"));
    }
    ...
    // Was "headingFieldsAndLabels()"/"linesFieldsAndLabels()" - no "get" prefix means neither
    // satisfies the JavaBean property naming convention EL property resolution requires, a
    // pre-existing dead reference independent of the @{...} binder issue (confirmed no other Java
    // callers). Also wrapped in ListModelList: the grids' "model" property is declared as
    // org.zkoss.zul.ListModel and AnnotateBinder has no automatic List->ListModel coercion.
    public List<Object> getHeadingFieldsAndLabels(){
        return new org.zkoss.zul.ListModelList<>(workReportTypeModel.getOrderedListHeading());
    }

    public List<Object> getLinesFieldsAndLabels() {
        return new org.zkoss.zul.ListModelList<>(workReportTypeModel.getOrderedListLines());
    }
    ...
    public class OrderedFieldsAndLabelsRowRenderer implements RowRenderer {
        ...
        public void render(Row row, Object data, int i) {
            ...
            String name = _t("Unallocated name");
            String type;
    ...
    private void showMessageSortedLabelsAndFields() {
        ...
        if (messagesForUserSortedLabelsAndFields != null) {
            messagesForUserSortedLabelsAndFields.showMessage(
                    Level.ERROR,
                    _t("Index fields and labels must be unique and consecutive"));
        }
    }
    ...
    protected String getEntityType() {
        return _t("Timesheets Template");
    }

    @Override
    protected String getPluralEntityType() {
        return _t("Timesheets Templates");
    }
    ...
    protected boolean beforeDeleting(WorkReportType workReportType) {
        if (thereAreWorkReportsFor(workReportType)) {
            Messagebox.show(
                    _t("Cannot delete timesheet template. There are some timesheets bound to it."),
                    _t("Warning"), Messagebox.OK, Messagebox.EXCLAMATION);
            return false;
        }
